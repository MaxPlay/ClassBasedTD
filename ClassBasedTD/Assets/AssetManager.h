...
namespace CBTD
{
    ...
    public:
        ...
        int RequestAsset(std::string file, class Scene& scene);
    ...
    inline AssetManager<T>::~AssetManager()
    {
        ...
        for (const std::string& i : m_AssetFiles)
    ...
    template<class T>
    inline int AssetManager<T>::RequestAsset(std::string file, class Scene& scene)
    {
    ...
    template<class T>
    inline void AssetManager<T>::ClearScene(Scene* scene)
    {
        int id = scene->GetId();
        ...
        for (const std::pair<int, std::vector<int>>& l : m_SceneLinks)
        {
            if (l.first == id)
                continue;
